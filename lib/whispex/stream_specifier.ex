defmodule Whispex.StreamSpecifier do
  @moduledoc false
  ...
  @type stream_type ::
          :video | :video_without_pics | :audio | :subtitle | :data | :attachments | nil
  ...
  def add_option(%__MODULE__{options: options} = stream_specifier, option_name)
  ...
  def add_option(%__MODULE__{options: options} = stream_specifier, option_name, argument)
      when is_binary(option_name) do
  ...
  def specifier_string(%__MODULE__{stream_type: :video_without_pics, stream_index: index}),
    do: "V:#{index}"
  ...
  def specifier_string(%__MODULE__{stream_type: :attachments, stream_index: index}),
    do: "t:#{index}"

  def specifier_string(%__MODULE__{program_id: pid, stream_index: nil}) when is_integer(pid),
    do: "p:#{pid}"

  def specifier_string(%__MODULE__{program_id: pid, stream_index: index}) when is_integer(pid),
    do: "p:#{pid}:#{index}"

  def specifier_string(%__MODULE__{stream_id: sid}) when is_integer(sid), do: "i:#{sid}"

  def specifier_string(%__MODULE__{metadata_key: key, metadata_value: nil}) when is_binary(key),
    do: "m:#{key}"

  def specifier_string(%__MODULE__{metadata_key: key, metadata_value: value}) when is_binary(key),
    do: "m:#{key}:#{value}"
  ...
  def command_arguments(%__MODULE__{options: [option | options]} = specifier, acc) do
    command_arguments(
      %__MODULE__{specifier | options: options},
      acc ++ arg_for_option(specifier, option)
    )
  end
